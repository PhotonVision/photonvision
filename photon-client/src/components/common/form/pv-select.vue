  ...
  <div class="flex flex-col gap-2 py-2 sm:flex-row sm:items-center sm:gap-3">
    ...
    <div class="min-w-0 sm:flex-1" :style="{ flexBasis: selectWidth }">
      <select-root v-model="value" :disabled="disabled">
        <select-trigger
          :id="selectId"
          class="focus-within:ring-pv-primary text-pv-on-surface data-placeholder:text-pv-on-surface/45 flex h-10 w-full items-center justify-between gap-3 rounded-xl border border-white/12 bg-black/15 px-3 text-left text-sm shadow-sm transition outline-none focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-offset-black/20 disabled:cursor-not-allowed disabled:opacity-50"
          :aria-label="label"
        >
          ...
          <select-icon class="text-pv-on-surface/70 shrink-0">
