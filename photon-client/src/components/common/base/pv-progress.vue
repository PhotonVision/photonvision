...
<template>
  ...
  >
    <ProgressIndicator
      ...
      :style="`width: ${progressValue}%; background-color: ${themeColor}; --progress-color: ${themeColor}`"
    >
...
<style scoped>
.indicator {
  color: oklch(from var(--progress-color) round(1.21 - L) 0 0);
}

@supports (color: contrast-color(red)) {
  .indicator {
    color: contrast-color(var(--progress-color));
  }
}
</style>
