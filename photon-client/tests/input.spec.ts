...
test("Camera Gain Slider won't go past max or min", async ({ page }) => {
  ...
  await page.getByTestId("camera-gain-text-field").locator("input").fill("1000");
  await page.getByTestId("camera-gain-text-field").locator("input").press("Enter");
  await expect(page.getByTestId("camera-gain-text-field").locator("input")).toHaveValue("100");
  ...
  await expect(page.getByTestId("camera-gain-text-field").locator("input")).toHaveValue("99");

  await page.getByTestId("camera-gain-text-field").locator("input").fill("-10");
  await page.getByTestId("camera-gain-text-field").locator("input").press("Enter");
  await expect(page.getByTestId("camera-gain-text-field").locator("input")).toHaveValue("0");
  ...
  // Make sure the value is actually properly limited, not just visually
  await page.getByRole("button", { name: "appended action" }).nth(2).click();
  await expect(page.getByTestId("camera-gain-text-field").locator("input")).toHaveValue("1");
