...
photon-serde works by decomposing every message into a few base types: these are `int8` `int16` `int32` `int64` `float32` `float64` `boolean`. These base-types are essentially copied directly into the packet.
...
Custom messages are essentially structs that pack 