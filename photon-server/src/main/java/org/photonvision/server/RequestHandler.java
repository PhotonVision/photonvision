...
public class RequestHandler {
    ...
    public static void onDeleteObjectDetectionModelRequest(Context ctx) {
        try {
            ...
            if (!Files.deleteIfExists(
                ...
                ctx.status(500);
                ctx.result("Error deleting rknn file of model: " + modelName);
                return;
            }
            ...
            if (!Files.deleteIfExists(
                ...
                ctx.status(500);
                ctx.result("Error deleting labels file of model: " + modelName);
                return;
            }
    ...
    public static void onRenameObjectDetectionModelRequest(Context ctx) {
        try {
            ...
            Pattern namePattern = Pattern.compile("^[a-zA-Z0-9]+-\\d+-\\d+-yolov[58][a-z]*$");

            if (!namePattern.matcher(newName).matches()) {
                ctx.status(400);
                ctx.result("The new name does not follow the naming convention.");
                logger.error("The new name does not follow the naming convention.");
                return;
            }

            // TODO move into neural network manager

            var modelPath = Paths.get(ConfigManager.getInstance().getModelsDirectory().toString(), oldName + ".rknn");
            var labelsPath = Paths.get(
