...
public class SwerveDrive {
    ...
    public SwerveDrive() {
        ...
        swerveDriveSim = new SwerveDriveSim(
            kDriveFF, DCMotor.getFalcon500(1), kDriveGearRatio, kWheelDiameter/2.0,
            kSteerFF, DCMotor.getFalcon500(1), kSteerGearRatio,
            kinematics);
    }
     ...
     * @param openLoop If swerve modules should use feedforward only and ignore velocity feedback control.
     */
    ...
    /**
     * Command the swerve drive to the desired chassis speeds by converting them to swerve module states and
     * using {@link #setModuleStates(SwerveModuleState[], boolean)}.
     * 
     ...
     * @param resetSimPose If the simulated robot pose should also be reset. This effectively teleports the
     *     robot and should only be used during the setup of the simulation world.
     */
    public void resetPose(Pose2d pose, boolean resetSimPose){
        if(resetSimPose) {
            swerveDriveSim.reset(pose, false);
            // we shouldnt realistically be resetting pose after startup, but we will handle it anyway for testing
            for(int i = 0; i < swerveMods.length; i++) {
    ...
    /**
     * Get the SwerveModuleState of each swerve module (speed, angle).
    ...
    /**
     * Get the SwerveModulePosition of each swerve module (position, angle).
     * The returned array order matches the kinematics module order.
     */
    ...
    /**
     * Get the Pose2d of each swerve module based on kinematics and current robot pose.
     * The returned array order matches the kinematics module order.
     */
    ...
    public void log(){
        ...
        SmartDashboard.putNumber(table+"Omega Degrees", Math.toDegrees(chassisSpeeds.omegaRadiansPerSecond));
        ...
        SmartDashboard.putNumber(table+"Target Omega Degrees", Math.toDegrees(targetChassisSpeeds.omegaRadiansPerSecond));
    ...
    public void simulationPeriodic(){
        ...
        for(int i = 0; i < swerveMods.length; i++) {
            ...
            swerveMods[i].simulationUpdate(
                drivePos, driveRate, driveCurrents[i],
    ...
    /**
     * The "actual" pose of the robot on the field used in simulation.
     * This is different from the swerve drive's estimated pose.
     */
