...
public class SwerveDriveSim {
    ...
    /**
     * Computes the new x given the old x and the control input. Includes the effect of static friction.
     * 
     ...
     */
    protected static Matrix<N2, N1> calculateX(Matrix<N2, N2> discA, Matrix<N2, N1> discB, Matrix<N2, N1> x, double input, double ks) {
        ...
        // non-zero system velocity, but input causes velocity sign change. Resist input after sign change
    ...
    /**
     * Reset the simulated swerve drive state. This effectively teleports the robot and should only
     * be used during the setup of the simulation world.
     * 
     * @param pose The new pose of the simulated swerve drive.
     * @param preserveMotion If true, the current module states will be preserved. Otherwise,
     *     they will be reset to zeros.
     */
     ...
     * @param moduleSteerStates The new states of the modules' steer systems in [radians, radians/sec].
     *     These should match the module order used in the kinematics.
     */
    public void reset(Pose2d pose, List<Matrix<N2, N1>> moduleDriveStates, List<Matrix<N2, N1>> moduleSteerStates) {
        if(moduleDriveStates.size() != driveStates.size() || moduleSteerStates.size() != steerStates.size())
     ...
     * the simulation world, without any noise. If you are simulating a pose estimator, this pose should
     * only be used for visualization or camera simulation. This should be called after {@link #update(double)}.
     */
    ...
    /**
     * Get the {@link SwerveModulePosition} of each module. The returned array order matches the kinematics module order.
     * This should be called after {@link #update(double)}.
     */
    public SwerveModulePosition[] getModulePositions() {
        var positions = new SwerveModulePosition[numModules];
        for(int i = 0; i < numModules; i++) {
            positions[i] = new SwerveModulePosition(driveStates.get(i).get(0, 0), new Rotation2d(steerStates.get(i).get(0, 0)));
        }
        return positions;
    }

    /**
     * Get the {@link SwerveModulePosition} of each module with rudimentary noise simulation.
     * The returned array order matches the kinematics module order. This should be called after
     * {@link #update(double)}.
     * 
    ...
    /**
     * Get the {@link SwerveModuleState} of each module. The returned array order matches the kinematics module order.
     * This should be called after {@link #update(double)}.
     */
    public SwerveModuleState[] getModuleStates() {
        var positions = new SwerveModuleState[numModules];
        for(int i = 0; i < numModules; i++) {
            positions[i] = new SwerveModuleState(driveStates.get(i).get(1, 0), new Rotation2d(steerStates.get(i).get(0, 0)));
        }
        return positions;
    }
    ...
    /**
     * Calculates the current drawn from the battery by the motor(s). Ignores regenerative current from back-emf.
     * 
    ...
    /**
     * Get the current draw in amps for each module's drive motor(s). This should be called after {@link #update(double)}.
     * The returned array order matches the kinematics module order.
     */
    ...
    /**
     * Get the current draw in amps for each module's steer motor(s). This should be called after {@link #update(double)}.
     * The returned array order matches the kinematics module order.
     */
    ...
    /**
     * Get the total current draw in amps of all swerve motors. This should be called after {@link #update(double)}.
     */
