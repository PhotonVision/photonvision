...
- A `Decimation` control (1x/2x/4x, default 2x for continuity with earlier versions of this pipeline) is available, as of vkapriltag v1.3.0 - lower values are more accurate but slower; the camera's resolution must be evenly divisible by whichever value is chosen. This pipeline type still has no `Blur` or `Refine Edges` controls, unlike the standard AprilTag pipeline: vkapriltag has no Gaussian pre-blur stage and does not perform edge refinement. These aren't configurable because they aren't implemented, not because they're hidden.
