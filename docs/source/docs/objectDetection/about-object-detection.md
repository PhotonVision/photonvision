...
In order to add a custom model to PhotonVision, copy the new `.rknn` model file and the labels file (which should contain one line per label the model outputs, with no training newline), into /home/pi/photonvision_config/models. This can be accomplished by SSHing into the coprocessor and using FTP/SFTP. You can also use WinSCP or FileZilla to access the coprocessor's filesystem.