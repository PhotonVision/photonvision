...
public class FindBoardCornersPipe  extends CVPipe<List<Mat>, List<List<Mat>>, FindBoardCornersPipe.FindCornersPipeParams> {
    ...
    public void createObjectPoints(){
        ...
        * */
        ...
        //Chessboard and dot board have different 3D points to project as a dot board has alternating dots per column
        if(params.isUsingChessboard){
            ...
            for (int i = 0; i < patternSize.height * patternSize.width; i++) {
                objectPoints.push_back(new MatOfPoint3f(new Point3((double)i / patternSize.width, i % patternSize.width, 0.0f)));
            }
        }else{
            //Here we need to alternate the amount of dots per column since a dot board is not rectangular and also by taking in account the grid size which should be in mm
            for( int i = 0; i < patternSize.height; i++ ) {
                for (int j = 0; j < patternSize.width; j++) {
                    objectPoints.push_back(new MatOfPoint3f( new Point3((2 * j + i % 2) * params.gridSize, i * params.gridSize, 0.0d)));
                }
            }
        }
    ...
    private boolean findBoardCorners(Mat frame){
        ...
        if(params.isUsingChessboard) {
            ...
            //For dot boards
            boardFound = Calib3d.findCirclesGrid(frame, patternSize, boardCorners, Calib3d.CALIB_CB_ASYMMETRIC_GRID);
        }
        ...
        // convert back to BGR
        Imgproc.cvtColor(frame, frame, Imgproc.COLOR_GRAY2BGR);
        // draw the chessboard, doesn't have to be different for a dot board since it just re projects the corners we found
    ...
    public static class FindCornersPipeParams {
        ...
        public FindCornersPipeParams(int boardHeight, int boardWidth, boolean isUsingChessboard, double gridSize ){
