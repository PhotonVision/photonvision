...
than 200lines of code that you can simply drop into your project.
   ...
   The library detects markers of dictionaries ARUCO_MIP_36h12
(https://sourceforge.net/projects/aruco/files/aruco_mip_36h12_dict.zip/download)
    and AprilTag 36h11.


 Simply add this file to your project to start enjoying Aruco.  Example of
Usage:
...
J.Romero-Ramirez, Rafael Muñoz-Salinas, Rafael Medina-Carnicer, Image and Vision
Computing, vol 76, pages 38-47, year 2018 2."Generation of fiducial marker
dictionaries using mixed integer linear programming",S. Garrido-Jurado, R. Muñoz
Salinas, F.J. Madrid-Cuevas, R. Medina-Carnicer, Pattern Recognition:51,
481-491,2016
 ...
 *ChangeLog:
 * version 5: adds support for  AprilTag 36h11. Use
MarkerDetector::detect(img,10,MarkerDetector::APRILTAG_36h11);
...
*/
#ifndef PHOTONVISION_PHOTON_TARGETING_SRC_MAIN_NATIVE_INCLUDE_PHOTON_ARUCO_NANO_H_
#define PHOTONVISION_PHOTON_TARGETING_SRC_MAIN_NATIVE_INCLUDE_PHOTON_ARUCO_NANO_H_
#include <array>
...
class TagDicts {
 private:
  static constexpr std::array<const uint64_t, 8178> AprilTag_36h11_Data = {
  ...
  static constexpr std::array<const uint64_t, 3469> ArUco_36h12_Data = {
 ...
 public:
  static constexpr std::span<const uint64_t> APRILTAG_36h11 =
      AprilTag_36h11_Data;
...
class Marker : public std::vector<cv::Point2f> {
 public:
  ...
  inline std::pair<cv::Mat, cv::Mat> estimatePose(
      cv::Mat cameraMatrix, cv::Mat distCoeffs, double markerSize = 1.0f) const;
};

class MarkerDetector {
 public:
  static inline std::vector<Marker> detect(
      const cv::Mat& img, unsigned int maxAttemptsPerCandidate = 10,
      std::span<const uint64_t> dictionary = TagDicts::ARUCO_MIP_36h12) {
    return internalDetect(img, maxAttemptsPerCandidate, dictionary);
  }

 private:
  static inline std::vector<Marker> internalDetect(
      const cv::Mat& inputMat, unsigned int maxAttemptsPerCandidate = 10,
      std::span<const uint64_t> dictionary = TagDicts::ARUCO_MIP_36h12);
  ...
  static inline int findMarkerId(const cv::Mat& mat, int& numRotationsNeeded,
                                 std::span<const uint64_t>& dictionary);
...
std::vector<Marker> MarkerDetector::internalDetect(
    const cv::Mat& inputMat, unsigned int maxAttemptsPerCandidate,
    std::span<const uint64_t> dictionary) {
  ...
  cv::adaptiveThreshold(grayMat, thresholdedMat, 255.0,
                        cv::ADAPTIVE_THRESH_MEAN_C, cv::THRESH_BINARY_INV, 7,
                        7);
  std::vector<std::vector<cv::Point>> foundContours;
  std::vector<cv::Point> approxContour;
  cv::RNG rng;
  cv::findContours(thresholdedMat, foundContours, cv::noArray(), cv::RETR_LIST,
                   cv::CHAIN_APPROX_NONE);
  cv::Mat sampledMarker(8, 8, CV_8UC1);
  for (unsigned int contourIdx = 0; contourIdx < foundContours.size();
       contourIdx++) {
    if (50 > int(foundContours[contourIdx].size())) continue;
    cv::approxPolyDP(foundContours[contourIdx], approxContour,
                     double(foundContours[contourIdx].size()) * 0.05, true);
  ...
  if (detectedMarkers.size() > 0) {
    ...
    cv::cornerSubPix(
        grayMat, pointsToSubPix, {winSize, winSize}, {-1, -1},
        {cv::TermCriteria::MAX_ITER | cv::TermCriteria::EPS, 12, 0.005});
    for (unsigned int markerIdx = 0; markerIdx < detectedMarkers.size();
         markerIdx++) {
      for (int pointIdx = 0; pointIdx < 4; pointIdx++) {
        detectedMarkers[markerIdx][pointIdx] =
            pointsToSubPix[markerIdx * 4 + pointIdx];
      }
    }
  }
...
void Marker::draw(cv::Mat& in, const cv::Scalar color) const {
  ...
  cv::rectangle(in, (*this)[1] - p2, (*this)[1] + p2, {0, 255, 0, 255},
                lineWidth);
  cv::rectangle(in, (*this)[2] - p2, (*this)[2] + p2, {255, 0, 0, 255},
                lineWidth);
  cv::Point2f cent(0, 0);
  for (auto& p : *this) {
    cent += p;
...
#endif  // PHOTONVISION_PHOTON_TARGETING_SRC_MAIN_NATIVE_INCLUDE_PHOTON_ARUCO_NANO_H_
