...
private:
    ...
    static inline  int    getMarkerId(const cv::Mat &bits,int &nrotations,std::span<const uint64_t> dict);
...
std::vector<Marker>  MarkerDetector::_detect(const cv::Mat &img, unsigned int maxAttemptsPerCandidate, Dict dict){
    ...
    for (unsigned int i = 0; i < contours.size(); i++)
    {
        ...
        for(int i=0;i<int(maxAttemptsPerCandidate) && marker.id==-1;i++){
            ...
            auto marker2=marker;
            if( i!=0) for(int c=0;c<4;c++) {marker2[c].x+=0.001;marker2[c].y+=0.001;}//if not first, alter corner location
            int pixelSum=0;
            ...
            int nrotations=0;
            marker.id=getMarkerId(bits,nrotations, dict==ARUCO_MIP_36h12 ? ARUCO_MIP_36h12_Dict_codes : APRILTAG_36h11_Dict_codes);
...
int MarkerDetector:: getMarkerId(const cv::Mat &bits, int &nrotations, std::span<const uint64_t> dict){
