...
public class CalibrationRotationPipeTest {
    ...
    public void testUndistortImagePointsWithRotation() {
        ...
        FrameStaticProperties frameProps = new FrameStaticProperties((int)coeffs.resolution.width, (int)coeffs.resolution.height, -1, coeffs);
        FrameStaticProperties rotatedFrameProps = frameProps.rotate(ImageRotationMode.DEG_90);
        ...
        Calib3d.undistortPoints(originalMatOfPoints, undistortedOriginalPoints, frameProps.cameraCalibration.getCameraIntrinsicsMat(), frameProps.cameraCalibration.getDistCoeffsMat());

        // Rotate the input points by 90 degrees
        Point[] rotatedInputPoints = new Point[originalPoints.length];
        for (int i = 0; i < originalPoints.length; i++) {
            rotatedInputPoints[i] = new Point(rotatedFrameProps.imageWidth - originalPoints[i].y, originalPoints[i].x);
        }
        ...
        Calib3d.undistortPoints(rotatedMatOfPoints, undistortedRotatedPoints, rotatedCoeffs.getCameraIntrinsicsMat(), rotatedCoeffs.getDistCoeffsMat());
        ...
        for (int i = 0; i < undistortedOriginalArray.length; i++) {
            expectedRotatedPoints[i] = new Point(rotatedFrameProps.imageWidth - undistortedOriginalArray[i].y, undistortedOriginalArray[i].x);
        }
