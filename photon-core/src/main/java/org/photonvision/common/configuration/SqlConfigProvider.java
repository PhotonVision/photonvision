...
public class SqlConfigProvider extends ConfigProvider {
    ...
    private LinkedMigrationStep buildMigrations() {
        LinkedMigrationStep step;
        step = LinkedMigrationStep.fromSql(null, 2, schema01);
        step = LinkedMigrationStep.fromMigrationFunction(step, 3, updateCameraConfig2027);
        step = LinkedMigrationStep.fromSql(step, 4, sql04);

        return step;
    }

    private MigrationFunction updateCameraConfig2027 =
            (conn) -> {
