        ...
        for (NeuralNetworkBackend backend : supportedBackends) {
            if (models.containsKey(backend)) {
                ...
                if (model.isPresent()) {
                    ...
                    var oldLabelsPath = Paths.get(modelsDirectory.toString(), oldName.replace(".rknn", "-labels.txt"));
                    var newModelPath = Paths.get(modelsDirectory.toString(), newName);
                    var newLabelsPath = Paths.get(modelsDirectory.toString(), newName.replace(".rknn", "-labels.txt"));
