...
public class NeuralNetworkModelManager {
     ...
     *
     * <p>{@link}{@link NeuralNetworkModelManager#discoverModels(File)} should always be called after
    ...
    public boolean addNewModel(File model, File labels) {
        ...
        var modelPath =
                Paths.get(ConfigManager.getInstance().getModelsDirectory().toString(), model.getName());
        var labelsPath =
                Paths.get(ConfigManager.getInstance().getModelsDirectory().toString(), labels.getName());
        ...
        for (NeuralNetworkBackend backend : supportedBackends) {
            if (models.containsKey(backend)) {
                ...
                if (model.isPresent()) {
                    ...
                    var newModelPath =
                            Paths.get(ConfigManager.getInstance().getModelsDirectory().toString(), newName);
                    var newLabelsPath =
                            Paths.get(
                                    ConfigManager.getInstance().getModelsDirectory().toString(),
